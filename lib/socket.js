...
class Socket extends EventEmitter {
  ...
  init (socket, authenticate) {
    ...
    socket._toaCloseListener = () => this.destroy()
    socket
      .on('error', (error) => {
  ...
  _onHandshake (data, authenticate) {
    if (isFn(authenticate)) {
      ...
      if (res.type !== 'request' || res.payload.method !== 'auth') {
        ...
        this.socket.end(this._encodeBuf(this._encodeMsg(error)))
        return this.destroy()
      }
      ...
      try {
        ...
        this.socket.end(this._encodeBuf(this._encodeMsg(msgObj)))
        return this.destroy()
      }
    } else {
  ...
  _flushCommand (msgBuf) {
    ...
    if (!this._writeBytes) this.emit('drain')
    else {
  ...
  notification (method, params) {
    this.ntfyCount++
    this._flushCommand(
      this._encodeBuf(this._encodeMsg(jsonrpc.notification(method, params))))
  }
  ...
  error (id, error) {
    this._flushCommand(
      this._encodeBuf(this._encodeMsg(jsonrpc.error(id, this.createError(error)))))
  }
  ...
  destroy () {
    ...
    if (this._iter) this._iter.destroy()
    this.socket.unpipe()
    ...
    if (!bufs.length || !this.socket.writable) clearSocket()
    else this.socket.write(Buffer.concat(bufs), clearSocket)

    for (let id of Object.keys(this.rpcPendingPool)) {
  ...
  [Symbol.iterator] () {
    ...
    this.on('close', () => this._iter.destroy())
    return this._iter
  }
}
...
class SocketIterator {
  ...
  destroy () {
    ...
    if (isFn(this._queue.first())) {
      let error = new Error('socket hang up')
      error.code = 'ECONNRESET'
      this._queue.shift()(error)
    }
    this._queue.reset()
  }
}
