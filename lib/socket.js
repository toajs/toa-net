...
class Socket extends EventEmitter {
    ...
    socket.resp
      ...
      .once('data', (message) => {
        ...
        if (authentication instanceof Auth) {
          ...
          // params: [signature]
          try {
            socket.token = authentication.verify(res.payload.params[0])
  ...
  request (method, params) {
    return thunk.call(this, (done) => {
      this.rpcCount++
      if (this.ended) throw new Error('Socket have been closed!')
  ...
  destroy () {
    this.ended = true
    this.socket.destroy()
    let rpcPendingPool = this.rpcPendingPool
...
class SocketIterator {
  ...
  next () {
    ...
    if (this._onLive()) {
      value = thunk((done) => {
        if (typeof this._queue[0] === 'object') {
...
Socket.prototype[Symbol.iterator] = function () {
  let mixQueue = new Queue()
  this.on('message', (message) => {
    if (typeof mixQueue[0] === 'function') {
      mixQueue.shift()(null, message)
    } else mixQueue.push(message)
  })
  return new SocketIterator(mixQueue, () => !this.socket.ended)
}
