...
class Socket extends EventEmitter {
  ...
  destroy () {
    ...
    Object.keys(this.rpcPendingPool)
      .forEach((id) => {
        this.rpcPendingPool[id].done(new Error('Socket have been closed!'))
      })
  ...
  [Symbol.iterator] () {
    ...
    this.on('close', (hadError) => {
      ...
      if (isFn(queue.first())) queue.shift()(new Error('The socket is closed!'))
    })
...
class SocketIterator {
  ...
  next () {
    ...
    if (this._isAlive()) {
      ...
      while (queue.length) {
        let fn = queue.shift()
        if (isFn(fn)) fn(new Error('The iterator is terminated!'))
      }
    }
