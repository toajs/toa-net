...
class Socket extends EventEmitter {
  ...
  get iterQueLen () {
    return this._iterQueue ? this._iterQueue.length : 0
  }
      ...
      .once('data', (data) => {
        ...
        if (isFn(authenticate)) {
          ...
          try {
            ...
            return socket.end(this._encode(msgObj))
          }
        } else {
          this._onMessage(res)
        }

        socket.resp.on('data', (data) => {
          this._onMessage(this._decode(data))
        })
      })
  ...
  _flushCommand (msgBuf) {
    ...
    if (!this._queue.length) this.emit('drain')
    return this
  }
  ...
  // Decode buffer to "JsonRpcParsed object"
  // Abstract method. Can be overridden.
  _decode (data) {
    return jsonrpc.parse(data.toString('utf8'))
  }
  ...
  notification (method, params) {
    this._flushCommand(this._encode(jsonrpc.notification(method, params)))
  }

  success (id, result) {
    this._flushCommand(this._encode(jsonrpc.success(id, result)))
  }

  error (id, error) {
    this._flushCommand(this._encode(jsonrpc.error(id, this.createError(error))))
  }
  ...
  [Symbol.iterator] () {
    ...
    let queue = new Queue()
    this._iterQueue = queue // should have better way to implement `iterQueLen`
