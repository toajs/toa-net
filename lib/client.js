...
class Client extends Socket {
  ...
  resendRpc () {
    let len = 0
    let bufs = []
    Command.sortIds(Object.keys(this.rpcPendingPool))
      .map((id) => {
  ...
  connect () {
    ...
    socket.removeListener('connect', socket._defaultConnectListener)
    socket.removeListener('close', socket._defaultCloseListener)
    socket._defaultCloseListener = () => this.reconnect()
    socket._defaultConnectListener = () => {
      this.attempts = 0
      this.RETRY_DELAY = 500
      this.emit('connect')
    }
    ...
    socket
      .on('close', socket._defaultCloseListener)
      .on('connect', socket._defaultConnectListener)
    return this
  }
